...
# 
# *The following code tutorial is mainly based on the [statsmodels documentation](https://www.statsmodels.org/v0.10.0/gam.html) about generalized additive models (GAM). To learn more about this method, review ["An Introduction to Statistical Learning"](https://www.statlearning.com/) from James et al. (2021).*
# 
# Generalized additive models allow us to use regression splines (and methods like smoothing splines and local regression) to deal with multiple predictors. In [](splines.ipynb), we discussed regression splines, which we create by specifying a set of knots, producing a sequence of basis functions, and then using least squares to estimate the spline coefficients.
...
# As an example: 
# 
# - A cubic spline (degree=3) with 4 knots (K=4) will have $df=4+3=7$ degrees of freedom. If we use an intercept, we need to add an aditional degree of freedom.
# - A cubic spline (degree=3) with 5 degrees of freedom will have $𝑘=5−3=2$ knots.
# 
# In our case, we want to fit a cubic spline (degree=3) with an intercept and three knots (K=3) for both features (we also could use different values). This equals $df=3+3+1=7$; these degrees of freedom are used up by an intercept, plus six basis functions.
# 
# The higher the degrees of freedom, the "wigglier" the spline gets because the number of knots is increased.
# 
# *In the case of natural (restricted) cubic splines: df=𝑘−1 if you specify the knots or 𝑘=df+1 if you specify the degrees of freedom.*
# 
...
# Note that optimal penalization weights alpha could be obtained through generalized k-fold cross-validation with [.select_penweight_kfold](https://www.statsmodels.org/dev/generated/statsmodels.gam.generalized_additive_model.GLMGam.select_penweight_kfold.html)
