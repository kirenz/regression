...
# transform data. To learn more about pandas, review this [short introduction to pandas](https://kirenz.github.io/pandas/intro.html).
# 
...
# create a new data object with the name df
...
# To convert to a ordinal variable with pandas use [CategoricalDtype](https://pandas.pydata.org/pandas-docs/stable/categorical.html) and provide the order of your levels.
...
# `df = df.dropna()`
# 
# However, be careful not to drop too many observations if just one variable is the cause for the
#  missing values. In that case, it could be reasonable to only drop the variable (if many values are missing):
# 
# `df = df.drop('variable', axis=1)`
# 
# Usually, we will use other methods to fill in missing values (e.g. imputate missing values by using the mean) instead of dropping them.
...
# We will cover the modelling process in the next sections of this book.
# 
